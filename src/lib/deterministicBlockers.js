export function getDeterministicBlockers(state) {
  const blockers = [];
  ...
  if (!state.enrollChecks.epConfirmed) {
    blockers.push({
  ...
  if (!state.enrollChecks.piiConsent) {
    blockers.push({
  ...
  if (!state.enrollChecks.planConfirm) {
    blockers.push({
  ...
  if (!state.enrollChecks.submitConsent) {
    blockers.push({
  ...
  if (!String(state.notes.planName || "").trim()) {
    blockers.push({
  ...
  if (!String(state.notes.effectiveDate || "").trim()) {
    blockers.push({
